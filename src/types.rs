...
impl MessageTag {
    ...
    fn validate(&self) -> Result<()> {
        ...
        if !valid_name {
            return Err(Error::InvalidMessageTag(
                "names must match ^[A-Za-z0-9_-]{1,32}$".into(),
            ));
        }
        if self.name.to_ascii_lowercase().starts_with("__lettermint") {
            return Err(Error::InvalidMessageTag(
                "names must not start with __lettermint".into(),
            ));
        }
        let valid_value = !self.value.is_empty()
            && self.value.len() <= 64
            && self
                .value
                .bytes()
        ...
        if !valid_value {
            return Err(Error::InvalidMessageTag(
                "values must match ^[A-Za-z0-9_-]{1,64}$".into(),
            ));
        }
        Ok(())
    }
...
pub(crate) fn validate_message_tags(tags: &[MessageTag], has_legacy_tag: bool) -> Result<()> {
    let maximum = if has_legacy_tag { 19 } else { 20 };
    if tags.len() > maximum {
        return Err(Error::InvalidMessageTag(format!(
    ...
    for tag in tags {
        ...
        if !names.insert(tag.name()) {
            return Err(Error::InvalidMessageTag(
                "names must be unique and case-sensitive".into(),
            ));
        }
    }
    Ok(())
